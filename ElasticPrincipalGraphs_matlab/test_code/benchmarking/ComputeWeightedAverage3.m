function [NodeClusterCenters,NodeClusterRelativeSize] = ComputeWeightedAverage3(X, partition, PointWeights, NumberOfNodes)
...
%
    ...
    for i=1:NumberOfNodes
        ...
        if(size(inds,1)>0)
            ...
            NodeClusterCenters(i,:) = sum(bsxfun(@times,X(inds,:),PointWeights(inds)))/sum(PointWeights);
            NodeClusterRelativeSize(i) = sum(PointWeights)/TotalWeight;
        end
    end
end