function [EmbeddedNodePositions, ElasticEnergy, partition, MSE,EP,RP] = PrimitiveElasticGraphEmbedment(X, NodePositions, ElasticMatrix, varargin)
...
%       same space as X.
%   ElasticMatrix - k-by-k symmetric matrix describing the connectivity and the elastic properties 
%   of the graph. Star elasticities (mu coefficients) are along the
%       main diagonal (non-zero entries only for star centers), and the
%       edge elasticity moduli are at non-diagonal elements.
...
%   'TrimmingRadius' is trimming radius, a parameter required for robust principal graphs
...
%   fields 'Nodes' - which nodes are optimized, 'Partition' -  pre-computed 
%   partitioning of X
...
for i=1:MaxNumberOfIterations
        ...
        if profile
        numberOfLocalPoints(size(numberOfLocalPoints,2)+1)=size(X,1);
        numberOfFits=numberOfFits+1;
        numberOfFullFits=numberOfFullFits+1;
        numberOfGraphNodes=numberOfGraphNodes+size(NodePositions,1);
        FractionOfGraphNodes(size(FractionOfGraphNodes,2)+1) = 1;
        TimeForFitting(size(TimeForFitting,2)+1) = toc;
        end
    ...
    %diff = abs(ElasticEnergyOld-ElasticEnergy)/ElasticEnergy;
    %diff = immse(NodePositions,NewNodePositions)/norm(NodePositions);
    ...
    ElasticEnergyOld = ElasticEnergy;
end
...
%% version 1.1
    ...
    SizeSubSet = size(NodeSubSet,2);
    AllPointIndices = 1:size(X,1);
    ...
    for i=1:MaxNumberOfIterations
        ...
        if verbose
            [partitionLocal, distsLocal] = PartitionData(XLocal, NodePositions(NodeSubSet,:), MaxBlockSize, SquaredXLocal, TrimmingRadius);
            [ElasticEnergy, MSE, EP, RP] =...
                ComputePrimitiveGraphElasticEnergy(XLocal, NodePositions, ElasticMatrix, partitionLocal, distsLocal, 0);
        else
            [partitionLocal] = PartitionData(XLocal, NodePositions(NodeSubSet,:), MaxBlockSize, SquaredXLocal, TrimmingRadius);
            ElasticEnergy = 0;
        end

        [NewNodePositions] =...
            FitGraph2DataGivenPartitionLocal(XLocal, PointWeightsLocal, NodePositions, SpringLaplacianMatrix,...
                partitionLocal, NodeSubSet);
           
        if profile
        numberOfLocalPoints(size(numberOfLocalPoints,2)+1)=size(XLocal,1);
        numberOfFits=numberOfFits+1;
        numberOfGraphNodes=numberOfGraphNodes+size(NodeSubSet,1);
        FractionOfGraphNodes(size(FractionOfGraphNodes,2)+1) = size(NodeSubSet,1)/size(NodePositions,1);
        TimeForFitting(size(TimeForFitting,2)+1) = toc;
        end
    ...
    ElasticEnergyOld = ElasticEnergy;
     ...
     CountNumberOfIterations(size(CountNumberOfIterations,2)+1)=i;   
     end
     end
...
%        inds = find(partition==i);
%        if(size(inds,1)>0)
%            NodeClusterCenters(i,:) = sum(X(inds,:))/size(inds,1);
%            NodeClusterRelativeSize(i) = size(inds,1)/TotalWeight;
%        end
%    end
...
function [NewNodePositions] =...
    FitGraph2DataGivenPartitionLocal(XLocal, PointWeightsLocal, NodePositions,...
    SpringLaplacianMatrix, partitionLocal, NodeSubSet)
%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%
    ...
    FirstAverage = mean(XLocal);
    
    %sumOfRSs = sum(NodeClusterRelativeSizes(NodeSubSet));
    
    %rs(:,:) = NodeClusterRelativeSizes(:,:);
    %rs(NodeSubSet,:) = NodeClusterRelativeSizeLocal(:,:);%*sumOfRSs;
    ...
    %rhs1 = bsxfun(@times, NodeClusterRelativeSizes(ComplementNodeSet),WeightedAverages(ComplementNodeSet,:));
    %rp = repmat(NodeClusterRelativeSizes(ComplementNodeSet),1,size(XLocal,2));
    %rhs1 = rp.*WeightedAverages(ComplementNodeSet,:);
