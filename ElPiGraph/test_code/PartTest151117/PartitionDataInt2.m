function part = ...
    PartitionDataInt2(data, graph, part, index)
%%%%%%%%%%%%%%%%%%%%%%%
...
%
    ...
    for i = 1:graph.MaxBlockSize:data.nPoints
        ...
        ind = i:last;
        % Calculate distances
        [part.dists(ind), part.partition(ind)] =...
            min(bsxfun(@minus, centrLength,  2 * (data.X(ind,:) * cent)), [], 2);
    end
    part.dists = part.dists + data.SquaredX;
    %Apply trimming
    ind = part.dists > graph.TrimmingRadius;
    part.partition(ind) = 0;
    part.dists(ind) = graph.TrimmingRadius;
end