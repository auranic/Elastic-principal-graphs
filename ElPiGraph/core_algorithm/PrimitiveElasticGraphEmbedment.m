   ...
   if profile == 1
    ...
    if ~graph.LocalSearch
        ...
        for i = 1:graph.MaxNumberOfIterations
            ...
            if verbose
                [ENERGY, MSE, UE, UR] =...
                    ComputePrimitiveGraphElasticEnergy(data, graph, part); %#ok<UNRCH>
            end
            ...
            if profile == 1
                numberOfLocalPoints(end + 1) = data.nPoints; %#ok<AGROW>
                numberOfFits = numberOfFits + 1;
                numberOfFullFits = numberOfFullFits + 1;
                numberOfGraphNodes = numberOfGraphNodes + graph.nNodes;
            ...
            if verbose
                display(sprintf(['Iteration %i, difference of node',...
                    ...
                    i, diff, ElasticEnergy, MSE, EP, RP)); %#ok<UNRCH>
            end
        ...
        for i=1:graph.MaxNumberOfIterations
            ...
            if verbose
                [ENERGY, MSE, UE, UR] =...
                    ComputePrimitiveGraphElasticEnergy(dataLocal,...
                    graph, partLocal); %#ok<UNRCH>
            end
            ...
            if profile == 1
                numberOfLocalPoints(end + 1) = dataLocal.nPoints; %#ok<AGROW>
                numberOfFits = numberOfFits + 1;
                numberOfGraphNodes = numberOfGraphNodes + length(graph.NodeSubSet);
                FractionOfGraphNodes(end + 1) = length(graph.NodeSubSet)...
                    / graph.nNodes; %#ok<AGROW>
                TimeForFitting(end + 1) = toc; %#ok<AGROW>
            end
    ...
    if verbose
        display(sprintf('E=%4.4f, MSE=%4.4f, EP=%4.4f, RP=%4.4f,',...
            ElasticEnergy, MSE, EP, RP)); %#ok<UNRCH>
    end
end
